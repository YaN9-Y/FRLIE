...
class Encoder(BaseNetwork):
    def __init__(self, base_channel_nums, init_weights=True):
        ...
        self.conv3 = nn.Sequential(
            nn.ReflectionPad2d(1),
            nn.Conv2d(base_channel_nums * 2, base_channel_nums * 2, 3, stride=1),
            nn.BatchNorm2d(base_channel_nums * 2),
            nn.ReLU(),
        )
...
class Decoder(BaseNetwork):
    def __init__(self, base_channel_nums, out_channels=3, init_weights=True):
        ...
        self.up1 = bilinear_block(in_nc=base_channel_nums*4, out_nc=base_channel_nums*2, act_type='relu', norm_type='batch')

        self.conv1 = conv_block(base_channel_nums*2, out_nc=base_channel_nums*2, kernel_size=3, act_type='relu', norm_type='batch', pad_type='reflect')
        ...
        self.up2 = bilinear_block(in_nc=base_channel_nums * 2, out_nc=base_channel_nums * 1, act_type='relu',
                                      norm_type='batch')

        self.conv2 = conv_block(base_channel_nums * 1, out_nc= out_channels, kernel_size=3, pad_type='reflect',act_type=None ,norm_type=None)
        if init_weights:
            self.init_weights('kaiming')
...
def bilinear_block(in_nc, out_nc, bias=True, norm_type=None, act_type='relu', mode='CNA'):
    '''
    Conv layer with padding, normalization, activation
    mode: CNA --> Conv -> Norm -> Act
        NAC --> Norm -> Act --> Conv (Identity Mappings in Deep Residual Networks, ECCV16)
    ...
    bup = nn.UpsamplingBilinear2d(scale_factor=2)
    p = nn.ReflectionPad2d(1)
    c = nn.Conv2d(in_nc, in_nc//2, kernel_size=3, stride=1, padding=0, bias=bias)
    a = act(act_type) if act_type else None
    if 'CNA' in mode:
        n = norm(norm_type, out_nc) if norm_type else None
        return sequential( bup, p, c, n, a)
    elif mode == 'NAC':
        if norm_type is None and act_type is not None:
            a = act(act_type, inplace=False)
            # Important!
